...
def runGrader(soln, pairs, bombs):
    ...
    for _ in range(ITERATION):
        ...
        while qloc > 0:
            ...
            if score < bestScore:
                bestScore = score
                if VERBOSE and ITERATION <= 5:  # Draw state whenever a better score is achieved
                    drawState(n, qloc, bombs, pairs, roadplan, score, queryOutputList)

        if qloc == 0 and LAST and ITERATION <= 10:
            drawState(n, qloc, bombs, pairs, roadplan, score, queryOutputList)
        print(f"Best score: {bestScore}")
        tscore *= bestScore ** (1/ITERATION)
    print(f"Evaluated score: {tscore }")
    return int(tscore)
